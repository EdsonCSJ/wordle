...
const App = () => {
  ...
  const handleValidateWord = useCallback(() => {
    const currentTryWords = letters.slice((currentRow - 1) * 5, (currentRow - 1) * 5 + 5)

    const currentTryWord = currentTryWords.map((current) => current.letter).join('')

    if (words.includes(currentTryWord.toLowerCase())) {
      let tempLetters = [...letters]

      currentTryWords.forEach((current, indexLetter) => {
        const indexLetterWord = word.indexOf(current.letter)
  ...
  const checkIsKeyDisabled = useCallback(
    (key) => {
      ...
      else if (letters.length >= currentRow * 5 && !isEnterPressed && key !== 'ENTER' && key !== 'BACKSPACE')
        return true
      else if (letters.length < currentRow * 5 && key === 'ENTER') return true
      else if (letters.length / 5 + 1 === currentRow && key === 'BACKSPACE') return true
      else return false
    },
  ...
  const getFeedbackColor = () => {
    if(feedbackMessage === 'Você ganhou!')
     return '#538D3E'
  ...
  return (
    ...
    >
      <div style={{ borderBottom: '1px solid rgb(58, 58, 60)', width: '100vw', textAlign: 'center' }}>
        <h1 style={{ margin: '5px' }}>Wordle</h1>
      </div>
      {feedbackMessage && <p style={{backgroundColor: getFeedbackColor(), borderRadius: 3, padding: 10}}>{feedbackMessage}</p>}
      <div
        style={{
          height: 420,
          width: 350,
      ...
      <div>
        <KeyboardRow initialIndex={0} finalIndex={10} {...{ handleKeyPress, checkIsKeyDisabled, letters }} />
        <KeyboardRow initialIndex={10} finalIndex={19} {...{ handleKeyPress, checkIsKeyDisabled, letters }} />
        <KeyboardRow initialIndex={19} finalIndex={30} {...{ handleKeyPress, checkIsKeyDisabled, letters }} />
      </div>
    </div>
  )
}
...
const KeyboardRow = ({ initialIndex, finalIndex, handleKeyPress, checkIsKeyDisabled, letters = [] }) => {
  const checkBagroundColor = (key) => {
    const wasTried = letters.some((letter) => letter.letter === key)
    const isInWrongPosition = letters.some((letter) => letter.letter === key && letter.status === 'WRONG-POSITION')
    const isInRightPosition = letters.some((letter) => letter.letter === key && letter.status === 'RIGHT-POSITION')
